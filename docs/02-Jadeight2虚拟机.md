...
> ⚠️ **本文档描述的是 v2 旧实现 `Jadeight2`（177 条展开式 opcode、文件头 LE + 指令流 BE、
> 12 字节 `FunctionSave` 头），只作对照与考古用。**
> **当前的虚拟机是 `Jadeight2ReWrite`（ISA v3）：内部机制见
> [08-VM机制与字节码格式.md](08-VM机制与字节码格式.md)，JIT 见 [14-JIT.md](14-JIT.md)，
> 指令表见 [06-指令集参考.md](06-指令集参考.md)。** 旧 `.bc` 无法被 v3 的 VM / `j8run` 装载
> （报 `cannot load v3 module`）。
